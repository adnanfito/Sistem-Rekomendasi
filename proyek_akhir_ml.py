...
Kode ini mengganti nama kolom User-ID menjadi userID pada DataFrame ratings dan users untuk konsistensi penamaan. Penjelasan rinci:

* `ratings.rename(columns={'User-ID': 'userID'}, inplace=True)`
Kode ini mengganti nama kolom `User-ID` menjadi `userID` pada DataFrame `ratings`. Penggunaan `inplace=True` memastikan perubahan diterapkan langsung pada DataFrame tanpa perlu membuat salinan baru.

* `users.rename(columns={'User-ID': 'userID'}, inplace=True)`
Kode ini mengganti nama kolom `User-ID` menjadi `userID` pada DataFrame users, lagi-lagi dengan `inplace=True` untuk menerapkan perubahan secara langsung.

* `ratings.rename(columns={'Book-Rating': 'bookRating'}, inplace=True)`
Kode ini mengganti nama kolom Book-Rating menjadi bookRating pada DataFrame `ratings`. Penggunaan `inplace=True` memastikan perubahan diterapkan langsung pada DataFrame tanpa membuat salinan baru.
"""
...
"""**Penjelasan :**
Kode ini mengganti nama beberapa kolom dalam DataFrame books untuk menyederhanakan penamaan dan meningkatkan konsistensi. Berikut perubahan yang dilakukan:

* `Book-Title` menjadi `title`
* `Book-Author` menjadi `author`
* `Year-Of-Publication` menjadi `year`
* `Publisher` menjadi `publisher`

Dengan `inplace=True`, perubahan diterapkan langsung pada DataFrame tanpa membuat salinan baru. Tujuannya adalah untuk membuat nama kolom lebih pendek dan konsisten.
"""
...
class OptimizedRecommenderNet(tf.keras.Model):
    def __init__(self, num_users, num_book, embedding_size, **kwargs):
        ...
        # Hidden layers for feature interaction
        self.dense1 = layers.Dense(256, activation='relu', kernel_regularizer=regularizers.l2(1e-6))
        self.dense2 = layers.Dense(128, activation='relu', kernel_regularizer=regularizers.l2(1e-6))
    ...
    def call(self, inputs):
        ...
        # Hidden layers dan Dropout
        x = self.dense1(x)
        x = self.dense2(x)
        x = self.dropout(x)  # Dropout setelah hidden layers

        # Output layer dengan activation sigmoid
        return self.output_layer(x)

"""Early stopping callback untuk menghentikan pelatihan jika overfitting terjadi"""
